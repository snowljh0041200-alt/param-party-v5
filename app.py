...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "baram-party-v11-final-secret")
...
def filled_total(post):
    ...
    filled = sum(1 for s in slots if s.get("char_id"))
...
def render_posts(posts, user):
    ...
    for post in posts:
        ...
        for slot in post.get("slots", []):
            ...
            label = esc(slot.get("char_label"))
            copy_lines.append(f"{slot.get('job')} - {slot.get('char_label') or '모집중'}")
            if label:
                mine_or_owner = slot.get("uid") == uid or post.get("owner_uid") == uid
                leave_btn = f"<button type='button' class='mini danger' onclick=\"leaveSlot('{pid}','{sid}')\">비우기</button>" if mine_or_owner else ""
                slot_rows.append(f"<div class='slot filled'><div><b>{job}</b><br><span>✅ {label}</span></div>{leave_btn}</div>")
            else:
        ...
        <article class='party-card post' data-post-id='{pid}' data-owner='{owner_flag}' data-participants='{esc(participants)}'>
          <div class='post-head'>
            <div>
              <span class='pill {"done" if status=="마감" else "open"}'>{status}</span>
              <span class='pill type'>{esc(post.get("category"))}</span>
            </div>
          ...
          <div class='actions'>
            ...
            <button type='button' onclick="openPartyChat('{pid}')">파티채팅 {len(post.get("party_chat", []))}</button>
...
<div class='panel'><label>모집 자리 추가</label><div class='quick'><select id='slotJob'>{% for job in jobs %}<option>{{ job }}</option>{% endfor %}</select><button type='button' class='ok' onclick='addSlot()'>추가</button></div><div id='slotsBox'></div></div>
...
<div id='partyModal' class='modal'><div class='panel chat-panel'><div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:8px'><b>💬 파티채팅</b><button type='button' class='mini gray' onclick='closePartyChat()'>닫기</button></div><div id='partyChatList' class='chat-list'></div><div class='chat-form'><input id='partyChatText' placeholder='메시지' maxlength='150'><button type='button' onclick='sendPartyChat()'>전송</button></div></div></div>
...
def create_post():
    ...
    post = {
        ...
        "category": category,
        "place": place,
        "channel": clean_channel(request.form.get("channel", "")),
...
def edit_post(post_id):
    ...
    def fn(d):
        ...
        if p:
            p["channel"] = clean_channel(request.form.get("channel", ""))
...
def join_slot():
    ...
    def fn(d):
        ...
        if any(slot.get("char_id") == ch["id"] for slot in post.get("slots", [])):
            return
        for slot in post.get("slots", []):
            if slot.get("id") == req.get("slot_id") and not slot.get("char_id"):
                slot["char_id"] = ch["id"]
                slot["uid"] = user["id"]
