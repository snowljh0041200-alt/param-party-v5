...
def now_text(): return datetime.now().strftime("%m/%d %H:%M")
...
def cleanup(data):
    ...
    for p in data.get("posts",[]):
        ...
        kept.append(p)
    data["posts"]=kept
    data["global_chat"]=data.get("global_chat",[])[-180:]
    return data
...
def closed_left(p):
    ...
    left=int(((dt+timedelta(hours=AUTO_DELETE_HOURS))-datetime.now()).total_seconds()//60)
...
def render_chat_rows(chats,uid):
    ...
    return "\n".join([f"<div class='msg{' mine' if c.get('uid')==uid else ''}'><div class='msg-meta'>{esc(c.get('label'))} · {esc(c.get('time'))}</div><div>{esc(c.get('text'))}</div></div>" for c in chats[-100:]])
...
def global_chat():
    ...
    mutate(lambda d:d["global_chat"].append({"uid":user["id"],"label":char_label(ch),"text":text,"time":datetime.now().strftime("%H:%M")}))
...
def party_chat(pid):
    ...
    mutate(lambda d: find_post(d,pid).setdefault("party_chat",[]).append({"uid":user["id"],"label":char_label(ch),"text":text,"time":datetime.now().strftime("%H:%M")}))
    return jsonify(ok=True)
