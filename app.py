...
def render_posts(posts, u, farm_items=None, admin=False):
    ...
    for p in posts:
        ...
        <article class='party-card post' data-post-id='{pid}' data-owner='{"1" if owner else "0"}' data-participants='{e(data_part)}'>
          ...
          <button class='owner-only danger' onclick="deletePost('{pid}')">삭제</button>
          </div>
        </article>
        """)
    return "".join(out)
...
function deletePost(pid){if(!confirm('삭제할까?'))return;fetch('/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({post_id:pid})}).then(r=>r.json()).then(x=>{toast(x.ok?'삭제됨':x.reason||'실패');refresh()})}
